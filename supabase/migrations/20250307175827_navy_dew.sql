/*
  # Add PDF content storage to documents table

  1. Changes
    - Add column for storing PDF content as text
    - Add column for PDF generation timestamp
    - Add 'generated' status to documents
    - Update RLS policies

  2. Security
    - Enable RLS
    - Add policies for authenticated users
*/

-- Add new columns for PDF storage
ALTER TABLE documents 
  ADD COLUMN IF NOT EXISTS pdf_content text,
  ADD COLUMN IF NOT EXISTS pdf_generated_at timestamptz;

-- Update the status check constraint to include 'generated'
ALTER TABLE documents DROP CONSTRAINT IF EXISTS documents_status_check;
ALTER TABLE documents ADD CONSTRAINT documents_status_check 
  CHECK (status = ANY (ARRAY['draft'::text, 'review'::text, 'approved'::text, 'archived'::text, 'generated'::text]));

-- Enable RLS if not already enabled
ALTER TABLE documents ENABLE ROW LEVEL SECURITY;

-- Add policies for authenticated users if they don't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'documents' AND policyname = 'Users can view documents'
  ) THEN
    CREATE POLICY "Users can view documents" ON documents
      FOR SELECT TO authenticated
      USING (true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'documents' AND policyname = 'Users can create documents'
  ) THEN
    CREATE POLICY "Users can create documents" ON documents
      FOR INSERT TO authenticated
      WITH CHECK (true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'documents' AND policyname = 'Users can update documents'
  ) THEN
    CREATE POLICY "Users can update documents" ON documents
      FOR UPDATE TO authenticated
      USING (true)
      WITH CHECK (true);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'documents' AND policyname = 'Users can delete documents'
  ) THEN
    CREATE POLICY "Users can delete documents" ON documents
      FOR DELETE TO authenticated
      USING (true);
  END IF;
END $$;