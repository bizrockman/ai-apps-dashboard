...
const Projects: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setError(t('projects.errors.loadFailed', 'Fehler beim Laden der Daten'));
  ...
  const handleCreate = async (data: Omit<Project, 'id' | 'code' | 'createdAt' | 'updatedAt'>) => {    
    try {
      ...
      setError(t('projects.errors.createFailed', 'Fehler beim Erstellen des Projekts'));
  ...
  const handleUpdate = async (data: Omit<Project, 'code' | 'createdAt' | 'updatedAt'>) => {
    ...
    try {
      ...
      setError(t('projects.errors.updateFailed', 'Fehler beim Aktualisieren des Projekts'));
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(t('projects.errors.deleteFailed', 'Fehler beim Löschen des Projekts'));
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-800">{t('projects.title', 'Projekte')}</h2>
            <p className="text-gray-600">{t('projects.description', 'Verwalten Sie Bauprojekte')}</p>
          </div>
        </div>
        <button
          onClick={() => {
        ...
        >
          ...
          <span>{t('projects.singular', 'Projekt')} {t('common.add', 'hinzufügen')}</span>
        </button>
      </div>
