...
import { useStrategyAI } from './useStrategyAI';
import StrategyForm from './StrategyForm';
import StrategyResponse from './StrategyResponse';
import { StrategyFormData } from './StrategyForm';

const StrategyAI: React.FC = () => {
  const { analyze, response, isAnalyzing, startNewAnalysis } = useStrategyAI();
  const [lastFormData, setLastFormData] = useState<StrategyFormData | null>(null);
  ...
  const handleNewAnalysis = () => {
    ...
    const firstInput = formRef.current?.querySelector('select, input') as HTMLElement;
    firstInput?.focus();
  };

  const handleAnalyze = (data: StrategyFormData) => {
          ...
          <div className="p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-gray-800">Strategy AI</h2>
                  <p className="text-sm text-gray-500">AI-powered strategic planning assistance</p>
                </div>
              </div>
              {response && (
                <button
